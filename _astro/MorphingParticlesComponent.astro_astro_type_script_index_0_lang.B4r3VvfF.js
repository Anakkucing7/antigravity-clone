                    ...
                    self.onmessage = function(e) {
                        try {
                            ...
                            const distanceFunction = function (point, imageData) {
                                const px = Math.min(Math.max(0, Math.round(point[0])), imageData.width - 1);
                                const py = Math.min(Math.max(0, Math.round(point[1])), imageData.height - 1);
                                const pixelRedIndex = (px + py * imageData.width) * 4;
                                const val = imageData.data[pixelRedIndex];
                                const pixel = (val !== undefined ? val : 255) / 255;
                            ...
                            for (let i = 0; i < pointsBase.length; i++) {
                                ...
                                if (!nearestPoint) {
                                    nearestPoint = points.length > 0 ? points[0] : [pointsBase[i][0], pointsBase[i][1]];
                                }
                                nearestPoints.push(
                                    nearestPoint[0] - 250,
                                    nearestPoint[1] - 250
                            ...
                            self.postMessage({ error: err.message, stack: err.stack, index: e.data ? e.data.index : 0 });
                        }
                    };
            `], { type: `application/javascript` }), o = URL.createObjectURL(a), s = new Worker(o); s.onmessage = function (e) { if (e.data && e.data.error) { console.error("Worker calculation error:", e.data.error, e.data.stack); s.terminate(); URL.revokeObjectURL(o); i(new Error(e.data.error)); return; } let { nearestPoints: t, index: n } = e.data; s.terminate(), URL.revokeObjectURL(o), r({ nearestPoints: t, index: n }) }, s.onerror = function (e) { console.error(`Worker error:`, e.message || e, e.lineno, e), s.terminate(), URL.revokeObjectURL(o), i(e) }, s.postMessage({ imageData: e, pointsBase: t, index: n, density: this.scene.density })
        })
    } createDataTexturePosition(e) { let t = new Float32Array(this.length * 4); for (let n = 0; n < this.count; n++) { let r = n * 4; t[r + 0] = e[n * 2 + 0] * (1 / 250), t[r + 1] = e[n * 2 + 1] * (1 / 250), t[r + 2] = 0, t[r + 3] = 0 } let n = new f(t, this.size, this.size, a, b); return n.needsUpdate = !0, n } createRenderTarget() { return new h(this.size, this.size, { wrapS: E, wrapT: E, minFilter: s, magFilter: s, texture: this.posTex, format: a, type: _, depthBuffer: !1, stencilBuffer: !1 }) } setPointsTextureFromIndex(e) { this.posNearestTex = this.createDataTexturePosition(this.nearestPointsData[e]), this.posNearestTex.needsUpdate = !0, this.simMaterial.uniforms.uPosNearest = this.posNearestTex } init() {
        this.size = 256, this.length = this.size * this.size, this.posTex = this.createDataTexturePosition(this.pointsData), this.posNearestTex = this.createDataTexturePosition(this.nearestPointsData[0]), this.rt1 = this.createRenderTarget(), this.rt2 = this.createRenderTarget(), this.renderer.setRenderTarget(this.rt1), this.renderer.setClearColor(0, 0), this.renderer.clear(), this.renderer.setRenderTarget(this.rt2), this.renderer.setClearColor(0, 0), this.renderer.clear(), this.renderer.setRenderTarget(null), this.noise = new v, this.simScene = new o, this.simCamera = new T(-1, 1, 1, -1, 0, 1), this.simMaterial = new r({
            uniforms: { uPosition: { value: this.posTex }, uPosRefs: { value: this.posTex }, uPosNearest: { value: this.posNearestTex }, uMousePos: { value: new n(0, 0) }, uRingRadius: { value: .2 }, uDeltaTime: { value: 0 }, uRingWidth: { value: .05 }, uRingWidth2: { value: .015 }, uIsHovering: { value: 0 }, uRingDisplacement: { value: this.scene.ringDisplacement }, uTime: { value: 0 } }, vertexShader: `
                void main() {
